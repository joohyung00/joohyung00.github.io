...
<!-- MY-PAGE:START (auto-generated listing — do not edit by hand) -->
{% raw %}
  ...
  function render() {
    ...
    itemsEl.innerHTML = slice.map(function (p) {
      return '<li><a href="' + esc(p.url) + '">' + highlight(p.title || p.slug, q) + "</a>"
        ...
        + (p.updated && p.updated !== p.created ? " · 수정 " + fmtDate(p.updated) : "") + "</div>"
